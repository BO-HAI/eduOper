...
if (!window.console) {
    (function (win) {
        var consoleModel = {},
            names = ['assert', 'clear', 'count', 'debug', 'dir', 'dirxml', 'error',
                'exception', 'group', 'groupCollapsed', 'groupEnd', 'inif', 'log',
                'notifyFirebug', 'profile', 'profileEnd', 'table', 'time', 'timeEnd', 'trace', 'warn'],
                i, len;

        for (i = 0, len = names.length; i < len; i++) {
            consoleModel[names[i]] = function () { };
        }

        window.console = consoleModel;
    }(window));
}
...
edu.config = {
    api: {
        ...
        analysisStatus: {
            ...
            statusObjs: [
                { key: '301', value: '用户凭据已过期' },
                { key: '302', value: '无效的用户凭据' },
                { key: '303', value: '需要用户凭据' },
                { key: '501', value: '没有开通课程' },
                { key: '502', value: '没有@s_label签订套餐协议@e_label' },
                { key: '231', value: '需要验证码' },
                { key: '230', value: '验证码错误' },
                { key: '0', value: '接口内部出现异常' },
                { key: '1', value: '请求成功并返回数据' },
                { key: '2', value: '请求参数异常' },
                { key: '3', value: '课程已过期' },
                { key: '101', value: '用户名不存在' },
                { key: '102', value: '密码错误' },
                { key: '103', value: '账户被冻结' },
                { key: '104', value: '邮箱不属于该用户' },
                { key: '105', value: '账户已删除' },
                { key: '106', value: '用户名已存在' },
                { key: '107', value: '未找到用户邮箱地址' },
                { key: '108', value: '用户邮箱已经存在' },
                { key: '109', value: '用户在其他地方已登录' },
                { key: '201', value: '非法用户名' },
                { key: '202', value: '非法密码' },
                { key: '203', value: '非法邮箱' },
                { key: '204', value: '非法手机号码' },
                { key: '205', value: '非法客户端' },
                { key: '206', value: '超出范围' },
                { key: '401', value: '请求失败' },
                { key: '402', value: '请求参数错误' },
                { key: '403', value: '请求数据无效' },
                { key: '404', value: '未找到请求数据' },
                { key: '405', value: '请求数据超出限制' }
            ]
        },
        valiReturnData: {
    ...
    tools: {
        ...
        formatDate: {
            ...
            regexp: {
                ...
                date: /(\d{4}-\d{1,2}-\d{1,3})T(\d{1,2}:\d{1,2}:\d{1,2})(\.{1}\d{1,3})/g, //版本2.3.1中放弃
    ...
    project: {
        //主项目
        api: { domain: 'http://api.edu24ol.com', testDomain: 'http://web.api.hq.com' },
        //日志项目
        log: { domain: 'http://log.api.edu24ol.com', testDomain: 'http://log.api.hq.com' },
        //短信项目
        sms: { domain: 'http://sms.api.edu24ol.com', testDomain: 'http://sms.api.edu24ol.com' },//无测试
        //产品项目
        product: { domain: 'http://product.api.edu24ol.com', testDomain: 'http://product.api.hq.com' },
        //评论项目
        comm: { domain: 'http://comm.api.edu24ol.com', testDomain: 'http://comm.api.hq.com' },
        //用户权限
        permission: { domain: 'http://up.api.edu24ol.com', testDomain: 'http://up.api.hq.com' },
        //公告
        notice: { domain: 'http://notice.api.edu24ol.com', testDomain: 'http://notice.api.hq.com' },
        //模考
        exam: { domain: 'http://exammongodb.api.edu24ol.com', testDomain: 'http://exammongodb.api.hq.com' },
        //作业
        lessonwork: { domain: 'http://lessonwork.api.edu24ol.com', testDomain: 'http://lessonwork.api.hq.com' },
        //答疑
        qa: { domain: 'http://ques.api.edu24ol.com', testDomain: 'http://ques.api.hq.com' },
        question: { domain: 'http://ques.api.edu24ol.com', testDomain: 'http://ques.api.hq.com' },
        //后台管理
        manage: { domain: 'http://manage.edu24ol.com', testDomain: 'http://manage.hq.com' }
    },

    oldTicket: 'Passport',//旧passport名称
    ticket: 'passport',//passport 名称
    regexp: {
        mail: /^[\w-]+(\.[\w-]+)*@[\w-]+(\.[\w-]+)+$/,
    ...
    sms: {
        requestObj: {
            ...
            type: 'post',
            postdata: { MobileNo: '1391', Message: 'info', Level: 1 },
            isResend: false
        }
    }
    , isIE6: function () {
...
edu.config.winLogin = edu.config.winLogin || {
    ...
    user_interface: '/user',//接口地址
    verifyCode_interface: 'http://api.edu24ol.com/VerifyCode?id=@id&type=1',//验证码接口地址
    test_verifyCode_interface: 'http://web.api.hq.com/VerifyCode?id=@id&type=1',//验证码接口测试地址
    getUserPwdURL: 'http://www.edu24ol.com/user_center/getpwd.asp',//忘记密码URL
    regUrl: 'http://www.edu24ol.com/classRegist.asp?url=' + window.location.href,//注册URL
    //第三方登录
    thirdLogin: [
        {
            id: 'qq',
    ...
    redirect: '/callback.html', //跨域POST提交回调地址（用户属性）
    title: '您需要登录后才可以继续', //弹窗标题（用户属性）
    hint: {
        a: '亲！好像发生了一个错误；编号：', //实际意义接口返回了错误编码
        b: '亲！网络好像不是很给力呀！'//edu.command.Api.apiRequest函数中发生了异常
    },
    dialog_param: { resizable: false, width: 400, modal: true },//jquery-ui-dialog参数（用户属性）
    button: 'edu_login_btn', // ['btn_login', 'login2'],//'login',//触发元素id （string or Array）（用户属性）
    winBodyId: 'edu-dialog',//承载元素id（用户属性）
    verifyCodeDivID: 'verifycode_div',//注册码DIV ID
    errorBoxDivID: 'errorInfo_div',//显示错误信息元素ID
    verifyCodeImgId: 'guid_mig',//验证码图片Id
    verifyLoadImgUrl: 'http://static.edu24ol.com/images/winLogin/load.jpg',//验证码加载中图片地址
    logining: '<img src="http://static.edu24ol.com/images/winLogin/login2.gif" / >',//加载效果
    //输入框名称
    input: {
        userNameId: 'username',
        passWordId: 'password',
    ...
    //状态
    status: {
        pwdError: { key: '102', value: '密码错误' },
        verifyError: { key: '230', value: '验证码错误' },
        needVerify: { key: '231', value: '需要验证码' }
    },//状态对象，记录返回状态
    css: {
        error: 'ui-state-error ui-widget-content' // 提示：红色边框
    },
...
edu.config.winLogin.createWindow = function () {
    var winBody = document.getElementById(edu.config.winLogin.winBodyId),
    //窗体内最外层div
    ...
    userNameLeftDiv = document.createElement('div'),
    userNameLeftDivLabel = document.createElement('label'),
    userNameRightDiv = document.createElement('div'),
    userNameRightDivInput = document.createElement('input'),
    //pwdDiv
    pwdDiv = document.createElement('div'),
    pwdLeftDiv = document.createElement('div'),
    pwdLeftDivLabel = document.createElement('label'),
    pwdRightDiv = document.createElement('div'),
    pwdRightDivInput = document.createElement('input'),
    pwdRightDivA = document.createElement('a'),
    //verifycodeDiv
    verifyDiv = document.createElement('div'),
    verifyLeftDiv = document.createElement('div'),
    verifyLeftDivLabel = document.createElement('label'),
    verifyRightDiv = document.createElement('div'),
    verifyRightDivInput = document.createElement('input'),
    verifyRightDivSpan = document.createElement('span'),
    verifyRightDivSpanImg = document.createElement('img'),
    verifyRightDivSpanSpan = document.createElement('span'),
    //登录div
    loginDiv = document.createElement('div'),
    loginLeftDiv = document.createElement('div'),
    loginLeftDivButton = document.createElement('button'),
    loginLeftDivSpan = document.createElement('span'),
    loginLeftDivLabel = document.createElement('label'),
    loginLeftDivA = document.createElement('a'),
    //errorDiv
    ...
    thirdDivSpan = document.createElement('span'),
    thirdDivSpanAImg,
    thirdDivSpanA, //循环创建这个a
    thirdObjs = edu.config.winLogin.thirdLogin,
    i, len;
    ...
    userNameLeftDiv.style.styleFloat = 'left';//ie
    userNameLeftDiv.style.cssFloat = 'left';//other
    ...
    userNameLeftDiv.appendChild(userNameLeftDivLabel);//为用户名左边div添加子元素
    //userNameDiv 子元素 右
    ...
    userNameRightDiv.appendChild(userNameRightDivInput);//为用户名右边div添加子元素
    //为userName添加子元素
    ...
    pwdLeftDiv.style.styleFloat = 'left';//ie
    pwdLeftDiv.style.cssFloat = 'left';//other
    ...
    pwdLeftDiv.appendChild(pwdLeftDivLabel);//为用户名左边div添加子元素
    //pwdDiv 子元素 右
    ...
    pwdRightDiv.appendChild(pwdRightDivInput);//为用户名右边div添加子元素
    ...
    verifyLeftDiv.style.styleFloat = 'left';//ie
    verifyLeftDiv.style.cssFloat = 'left';//other
    ...
    verifyLeftDiv.appendChild(verifyLeftDivLabel);//为用户名左边div添加子元素
    //verifyDiv 子元素 右
    ...
    verifyRightDivSpan.appendChild(verifyRightDivSpanImg);//为 verifyRightDivSpan添加子元素
    verifyRightDivSpan.appendChild(verifyRightDivSpanSpan);

    verifyRightDiv.appendChild(verifyRightDivInput);//为用户名右边div添加子元素
    ...
    loginLeftDivA.style.fontSize = '12px',
    loginLeftDivA.style.marginLeft = '5px';
...
edu.config.BuildUrl = function (api) {
    ...
    this.getQAUrl = function () {
        return api.createFullUrl(createObj('qa', _path, _passport));
    };
};